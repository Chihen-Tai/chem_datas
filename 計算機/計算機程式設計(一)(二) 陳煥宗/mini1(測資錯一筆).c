...
void handle_binary_op(AST *node);
void handle_unary_op(AST *node);
...
AST *parse(Token *arr, int l, int r, GrammarState S)
{
	...
	case UNARY_EXPR:
		...
		if (arr[l].kind == PLUS || arr[l].kind == MINUS || arr[l].kind == PREINC || arr[l].kind == PREDEC) // fix --x ++x bug
		{
...
void semantic_check(AST *now)
{
	...
	if (now->kind == PREINC || now->kind == PREDEC || now->kind == POSTINC || now->kind == POSTDEC)//for x++ x-- ++x --x
	{
		AST *tmp = now->mid;
		while (tmp->kind == LPAR)
			tmp = tmp->mid;
	...
	// 		err("Operand of ADD/SUB/MUL/DIV/REM must be an identifier or constant.");
	// }
...
static int reg_count = 0; // reset register count
void codegen(AST *root)
{
	if (root == NULL)
		return;
	...
	case IDENTIFIER:
	{
		index = (root->val - 'x') * 4; // Map 'x', 'y', 'z' to memory addresses. 0 4 8
		printf("load r%d [%d]\n", reg_count, index);
		root->val = index;//record the memory address
		break;
	}
	...
	case ASSIGN:
		codegen(root->rhs); // Right side first
		cur_reg = reg_count++;
		codegen(root->lhs); // Then left side (variable)
		printf("store [%d] r%d\n", root->lhs->val, cur_reg);
		reg_count--;
		break;
	...
	case PREDEC:
	case POSTDEC:
		handle_unary_op(root);
		break;

	case LPAR:
		codegen(root->mid); // Expression within parentheses.
		break;
	...
	default:
		err("Unsupported AST node type in code generation.");
	}
}

void handle_binary_op(AST *root)
{
	int cur_reg;//current register
	codegen(root->lhs);//lhs
	cur_reg = reg_count++;
	codegen(root->rhs);//rhs

	switch (root->kind)
	{
	case ADD:
		printf("add r%d r%d r%d\n", cur_reg, cur_reg, cur_reg + 1);
		break;
	case SUB:
	...
	case DIV:
		...
		break;
	case REM://remainder of division==mod
...
void handle_unary_op(AST *root)
{
	codegen(root->mid);
	int cur_reg = reg_count;
	// printf("1\n");
	if (root->kind == PREINC) //++x
	{
		printf("add r%d r%d 1\n", cur_reg, cur_reg);		 // Increment
		printf("store [%d] r%d\n", root->mid->val, cur_reg); // Store the updated value
	}
	else if (root->kind == POSTINC) // x++
	{
		printf("add r%d r%d 1\n", cur_reg, cur_reg);		 // Increment
		printf("store [%d] r%d\n", root->mid->val, cur_reg); // Store the updated value
		printf("sub r%d r%d 1\n", cur_reg, cur_reg);		 // Decrement
	}
	else if (root->kind == PREDEC) //--x
	{
		printf("sub r%d r%d 1\n", cur_reg, cur_reg);		 // Decrement
		printf("store [%d] r%d\n", root->mid->val, cur_reg); // Store the updated value
	}
	else if (root->kind == POSTDEC) // x--
	{
		printf("sub r%d r%d 1\n", cur_reg, cur_reg);		 // Decrement
		printf("store [%d] r%d\n", root->mid->val, cur_reg); // Store the updated value
		printf("add r%d r%d 1\n", cur_reg, cur_reg);		 // Increment
